...
Shader "Skybox/ProceduralSkyboxCustom" {
Properties {
	...
	_StarScale("Star Scale", Range(1,1000)) = 200
	_StarPower("Star Power", Range(30,50)) = 40
}
...
SubShader {
	...
	// float4 StarsSubGraph(float _Star_Scale, float _Star_Power, float3 WorldSpacePosition)
	// {
	// 	float4 Color_f = IsGammaSpace() ? LinearToSRGB(float4(766.9962, 725.9457, 614.3639, 1)) : float4(766.9962, 725.9457, 614.3639, 1);
	// 	float3 _Normalize;
	// 	Unity_Normalize_float3(IN.WorldSpacePosition, _Normalize);
	// 	float _Split_R = _Normalize[0];
	// 	float _Split_G = _Normalize[1];
	// 	float _Split_B = _Normalize[2];
	// 	float _Split_A = 0;
	// 	float2 _Vector2 = float2(_Split_R, _Split_B);
	// 	float2 _Divide;
	// 	Unity_Divide_float2(_Vector2, (_Split_G.xx), _Divide);
	// 	float _Property = _Star_Scale;
	// 	float _SimpleNoise;
	// 	Unity_SimpleNoise_Deterministic_float(_Divide, _Property, _SimpleNoise);
	// 	float _Property = _Star_Power;
	// 	float _Power;
	// 	Unity_Power_float(_SimpleNoise, _Property, _Power);
	// 	float4 _Multiply;
	// 	Unity_Multiply_float4_float4(Color_f, (_Power.xxxx), _Multiply);
	// 	return _Multiply;
	// }
}
